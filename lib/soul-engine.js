/**
 * Universal Soul Engine
 * 地理データ、ロジックJSON、プレイヤー入力を合成して「魂（Identity）」を生成する
 */

class SoulEngine {

    /**
     * キャラクターのスポーン（ミント）処理
     * @param {Object} geoPoint   - 土地JSON（izu.json等）の特定のヘックスのattr
     * @param {Object} logic      - ロジックJSON (sengoku-logic.json)
     * @param {Object} legacyData - 前代からの引き継ぎデータ（任意）
     * @param {Object} userPrefs  - ユーザーが選択した設定（gender等, 任意）
     */
    static spawn(geoPoint, logic, legacyData = null, userPrefs = {}) {
        const { attr } = geoPoint;

        // 1. 性別の決定
        const gender = this._determineAttribute(
            logic.generation_protocols.gender,
            userPrefs.gender
        );

        // 2. 年齢の決定
        const age = this._determineAge(
            logic.generation_protocols.spawn_age,
            userPrefs.age
        );

        // 3. 職業の抽選（地形補正を適用）
        // fix #03: logic.province → attr.province に修正
        const occupation = this._determineOccupation(
            attr,
            logic.spawn_probability,
            logic.province_overrides[attr.province] // 地域補正があれば適用
        );

        // 4. 行動AP補正の計算（性別 × 職業 × 遺産）
        const apModifiers = this._calculateAPModifiers(
            gender,
            occupation,
            logic,
            legacyData
        );

        // 5. 最大APと学習効率（年齢数式適用）
        // fix #04: JSONの数式文字列から動的に計算
        const stats = this._calculateAgeStats(age, logic.trait_modifier_map.age_traits);

        // 6. ビジュアルキーの紐付け（疎結合アセット用）
        const visualKey = this._getVisualKey(gender, occupation, age, logic);

        return {
            timestamp: Date.now(),
            spawn_location: geoPoint.hex_id,
            identity: {
                gender,
                age,
                occupation,
                bloodline: legacyData ? legacyData.unlocked_bloodline : "commoner"
            },
            capabilities: {
                ap_modifiers: apModifiers,
                max_ap: stats.max_ap,
                learning_rate: stats.learning_rate
            },
            visual_ref: visualKey,
            legacy_status: "active"
        };
    }

    // 重み付け抽選
    static _weightedRandom(pool) {
        let totalWeight = pool.reduce((sum, item) => sum + (item.weight || item.base_weight), 0);
        let random = Math.random() * totalWeight;
        for (let item of pool) {
            random -= (item.weight || item.base_weight);
            if (random <= 0) return item.id;
        }
        return pool[0].id;
    }

    // 性別等の属性決定
    static _determineAttribute(protocol, preferredValue) {
        if (protocol.input_method === "choice" && preferredValue) return preferredValue;
        return this._weightedRandom(protocol.options);
    }

    // 年齢決定
    static _determineAge(protocol, preferredValue) {
        if (protocol.input_method === "discrete" && preferredValue && protocol.available_formats.values.includes(preferredValue)) {
            return preferredValue;
        }
        const vals = protocol.available_formats.values;
        return vals[Math.floor(Math.random() * vals.length)];
    }

    // 職業決定ロジック
    static _determineOccupation(attr, prob, provinceOverride) {
        let pool = JSON.parse(JSON.stringify(prob.occupation_pool)); // ディープコピー

        // 地形補正の適用
        Object.values(prob.terrain_modifiers).forEach(mod => {
            let match = true;
            if (mod.condition.terrain_type !== undefined && mod.condition.terrain_type !== attr.terrain_type) match = false;
            if (mod.condition.elevation_m) {
                if (mod.condition.elevation_m.lt && attr.elevation_m >= mod.condition.elevation_m.lt) match = false;
            }

            if (match) {
                pool.forEach(occ => {
                    if (mod.targets.includes(occ.id)) occ.base_weight *= mod.mul;
                });
            }
        });

        // 地域（国）補正の適用
        if (provinceOverride && provinceOverride.samurai_spawn_bonus) {
            pool.find(o => o.id === "samurai").base_weight *= provinceOverride.samurai_spawn_bonus;
        }

        return this._weightedRandom(pool);
    }

    // AP補正計算
    static _calculateAPModifiers(gender, occupation, logic, legacy) {
        const genderBase = logic.trait_modifier_map.gender_traits[gender].ap_mult;
        const occBase = logic.occupation_ap_modifiers[occupation];

        let combined = {};
        const allActions = new Set([...Object.keys(genderBase), ...Object.keys(occBase)]);

        allActions.forEach(action => {
            const gMul = genderBase[action] || 1.0;
            const oMul = occBase[action] || 1.0;
            let finalMul = gMul * oMul;

            // 遺産（Legacy）による引き継ぎ補正
            if (legacy && legacy.inheritance_ap_bonus) {
                finalMul *= (1.0 - legacy.inheritance_ap_bonus); // 例: 0.1ボーナスならコスト0.9倍
            }

            combined[action] = parseFloat(finalMul.toFixed(2));
        });

        return combined;
    }

    // 年齢によるステータス計算
    // fix #04: ハードコードをやめ、JSONの数式文字列を安全にパース
    static _calculateAgeStats(age, traits) {
        const max_ap = this._evalFormula(traits.ap_max_formula, { age });
        const learning_rate = this._evalFormula(traits.learning_rate_formula, { age });
        return { max_ap, learning_rate };
    }

    /**
     * 数式文字列を安全に評価する
     * 対応演算子: +, -, *, /, max(), min()
     * @param {string} formula - 数式文字列（例: "100 + (age * 2)"）
     * @param {Object} vars    - 変数マップ（例: { age: 13 }）
     */
    static _evalFormula(formula, vars) {
        // 変数を数値に置換
        let expr = formula;
        for (const [key, val] of Object.entries(vars)) {
            expr = expr.replace(new RegExp(`\\b${key}\\b`, "g"), val);
        }

        // max(a, b) / min(a, b) を括弧ネスト対応で解決
        // [^)]+ では内側に括弧を含む引数を正しく切り出せないため、
        // 対応する閉じ括弧を自前で探して引数を取り出す
        expr = this._resolveMaxMin(expr);

        // 四則演算のみ許可する安全なパース
        if (!/^[\d\s\+\-\*\/\(\)\.]+$/.test(expr)) {
            throw new Error(`Unsafe formula expression: ${expr}`);
        }

        return parseFloat(Function(`"use strict"; return (${expr})`)());
    }

    /**
     * max(...) / min(...) を再帰的に数値へ解決する
     * 内側に括弧を含む引数（例: 1.5 - (age * 0.05)）にも対応
     */
    static _resolveMaxMin(expr) {
        // max または min が現れる位置を探してループ処理
        const re = /\b(max|min)\(/g;
        let match;
        while ((match = re.exec(expr)) !== null) {
            const fn = match[1];
            const start = match.index + fn.length + 1; // '(' の直後

            // 対応する ')' を括弧カウントで探す
            let depth = 1;
            let i = start;
            while (i < expr.length && depth > 0) {
                if (expr[i] === '(') depth++;
                else if (expr[i] === ')') depth--;
                i++;
            }
            const argsStr = expr.slice(start, i - 1); // 括弧内の文字列

            // 引数を ',' で分割（括弧の外側のカンマのみ対象）
            const args = this._splitArgs(argsStr);
            if (args.length !== 2) {
                throw new Error(`max/min requires exactly 2 arguments: ${argsStr}`);
            }

            // 各引数を再帰的に解決してから Math.max/min を適用
            // parseFloat では '1.5 - (13 * 0.05)' のような式を評価できないため
            // max/min を解決した後、Function で四則演算ごと評価する
            const resolvedA = this._resolveMaxMin(args[0].trim());
            const resolvedB = this._resolveMaxMin(args[1].trim());
            const a = parseFloat(Function('"use strict"; return (' + resolvedA + ')')());
            const b = parseFloat(Function('"use strict"; return (' + resolvedB + ')')());
            const result = fn === 'max' ? Math.max(a, b) : Math.min(a, b);

            // 元の式を計算結果で置換してやり直し
            expr = expr.slice(0, match.index) + result + expr.slice(i);
            re.lastIndex = 0; // 置換後は先頭から再スキャン
        }
        return expr;
    }

    /**
     * 括弧の外側のカンマで引数文字列を分割する
     */
    static _splitArgs(argsStr) {
        const args = [];
        let depth = 0;
        let current = '';
        for (const ch of argsStr) {
            if (ch === '(') { depth++; current += ch; }
            else if (ch === ')') { depth--; current += ch; }
            else if (ch === ',' && depth === 0) {
                args.push(current);
                current = '';
            } else {
                current += ch;
            }
        }
        if (current) args.push(current);
        return args;
    }

    static _getVisualKey(gender, occupation, age, logic) {
        // 疎結合アセットJSONを引くためのキーを生成
        // 例: "m_base_samurai_13"
        const genderKey = logic.trait_modifier_map.gender_traits[gender].visual_key;
        return `${genderKey}_${occupation}_${age}`;
    }
}

export default SoulEngine;
